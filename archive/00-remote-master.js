...
export async function main(ns) {
	const allServers = findAllServers(ns, ns.getHostname());
...
async function decideAndExecute(ns, targetServer) {
	if (isScriptRunningOnRemote(ns, targetServer)) {
	...
	if (shouldWeaken(ns, targetServer)) {
		ns.exec('00-weaken.js', ns.getHostname(), calculateThreads(ns, 'weaken', targetServer), targetServer);
	} else if (shouldGrow(ns, targetServer)) {
		ns.exec('00-grow.js', ns.getHostname(), calculateThreads(ns, 'grow', targetServer), targetServer);
	} else if (shouldHack(ns, targetServer)) {
		ns.exec('00-hack.js', ns.getHostname(), calculateThreads(ns, 'hack', targetServer), targetServer);
	}
}

function isScriptRunningOnRemote(ns, targetServer) {
	return ns.ps(ns.getHostname()).some(p => p.args.includes(targetServer));
}
...
function calculateThreads(ns, scriptType, server) {
	...
	const scriptRam = ns.getScriptRam(`00-${scriptType}.js`, ns.getHostname());
